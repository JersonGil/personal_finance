...
function enrichTransactions(raw: Transaction[], categories: Category[]): EnrichedTransaction[] {
  ...
  return raw.map((t) => {
    ...
    return match
      ? {
          ...t,
          categoryMeta: { id: match.id, color: match.color, icon: match.icon, type: match.type },
        }
      : t;
  });
}
...
export function useAllTransactions(options?: UseAllTransactionsOptions) {
  ...
  const createTransaction = async (
    input: Pick<Transaction, 'type' | 'amount' | 'category' | 'description' | 'date'>,
  ) => {
