...
export default function TransactionsRealtime({ userId }: { userId: string | null }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('realtime-transactions')
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'transactions', filter: `user_id=eq.${userId}` }, (payload) => {
        const newTx = payload.new as Transaction
        update(newTx.id, newTx)
      })
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'transactions', filter: `user_id=eq.${userId}` }, (payload) => {
