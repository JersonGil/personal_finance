...
export const useTransactionsStore = create<TransactionsState>((set, get) => ({
  ...
  const income = transactions.filter((t: Transaction) => t.type === 'income').reduce((s: number, t: Transaction) => s + (isNaN(t.amount) ? 0 : t.amount), 0)
  const expenses = transactions.filter((t: Transaction) => t.type === 'expense').reduce((s: number, t: Transaction) => s + (isNaN(t.amount) ? 0 : t.amount), 0)
